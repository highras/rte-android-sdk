...
//
    ...
    ArrayList<Long>  wantLongList(Message message,String key) {
        ...
        try {
            ...
            for (Object value : attrsList) {
                if (value instanceof Integer)
                    retlist.add(((Integer) value).longValue());
                else if (value instanceof Long)
                    retlist.add(((Long) value).longValue());
                else if (value instanceof BigInteger)
                    retlist.add(((BigInteger) value).longValue());
                else
                    retlist.add(Long.valueOf(String.valueOf(value)));
            }
        }
        catch (Exception e)
        {
    ...
    ArrayList<Integer> wantIntList(Message message,String key) {
    ...
    HashMap<Long, List<Integer>> wantDeviceOption(Message message, String key) {
        HashMap<Long, List<Integer>> options = new HashMap<>();
        try {
    ...
    boolean wantBoolean(Message quest,String key) {
        Object obj = null;
        try {
            obj = quest.want(key);
        }
    ...
    ArrayList<Long> longList(Object values) {
...
//            Log.e("rtmsdk","wantLong NoSuchElementException " + key);
    ...
    int wantInt(Message quest,String key) {
        int value = -1;
        try {
            Object obj = quest.want(key);
            value = wantInt(obj);
    ...
    String bytesToHexString(byte[] bytes, boolean isLowerCase) {
